function start() {
  ...
  function movejogador() {
    ...
    if (jogo.pressionou[TECLA.S]) {
      disparo(DIRECAO_TIRO.RETO);
    }
    if (jogo.pressionou[TECLA.A]) {
      disparo(DIRECAO_TIRO.CIMA);
    }
    if (jogo.pressionou[TECLA.D]) {
      disparo(DIRECAO_TIRO.BAIXO);
    }
  }
  ...
  function moveinimigo2() {
    ...
    $("#teste").css("left", posicaoX - velocidade);
    console.log($("#teste").css("left"));
    if (posicaoX <= 0) {
  ...
  function disparo(direcao) {
    if (podeAtirar == true) {
      ...
      $("#disparo").css("transform", "rotate(" + angulo + "deg)");

      var tempoDisparo = window.setInterval(executaDisparo, 100, direcao);
    }

    function executaDisparo(direcao) {
      let posicaoX = parseInt($("#disparo").css("left"));

      let posicaoY = parseInt($("#disparo").css("top"));

      $("#disparo").css("left", posicaoX + 50);
      console.log(posicaoY + direcao * 10);
      $("#disparo").css("top", posicaoY + direcao * 10);
